...
const copy = (source, destination, droppableSource, droppableDestination) => {
  ...
  destClone.splice(droppableDestination.index, 0, { ...item, id: 4587 });
  return destClone;
};
class Editor extends Component {
  ...
  RenamePage = data => {
    ...
    if (pagetype === "Subheader") {
      ...
      this.setState(state => {
        ...
        var itemIndex = state.pages[subIndex].items.findIndex(
          obj => obj.id === itemId
        );
  ...
  doRemove = data => {
    ...
    if (pagetype === "Subheader") {
      ...
      var itemIndex = this.state.pages[subIndex].items.findIndex(
        obj => obj.id === itemId
      );
      this.setState(state => {
  ...
  removeStep(id, index) {
    var itemIndex = this.state.currentPage.page.content[0].steps.findIndex(
      obj => obj.id === id
    );
    this.setState(state => {
      const list = this.state.currentPage.page.content[0].steps.splice(
        itemIndex,
        1
      );
      return {
  ...
  addStep(step, stepperId) {
    ...
    let stepperIndex = this.state.currentPage.page.content.findIndex(
      obj => obj.id === stepperId
    );
    console.log(stepperIndex, stepperId);
    this.setState(state => {
      const list = state.currentPage.page.content[stepperIndex].steps.splice(
        step + 1,
        0,
        newStep
      );
  ...
  TextAction(data) {
    ...
    let textIndex = this.state.currentPage.page.content.findIndex(
      obj => obj.id === textId
    );
    this.setState(state => {
  ...
  expansionText(data) {
    ...
    var componentIndex = this.state.currentPage.page.content.findIndex(
      obj => obj.id === expansionId
    );

    var stepIndex = this.state.currentPage.page.content[
      componentIndex
    ].steps.findIndex(obj => obj.id === stepId);
    this.setState(state => {
      var list = (state.currentPage.page.content[componentIndex].steps[
        stepIndex
      ][sec] = value);
      return {
        list
  ...
  AlertAction(data) {
    ...
    var index = this.state.currentPage.page.content.findIndex(
      obj => obj.id === alertId
    );
    this.setState(state => {
  ...
  stepperText(data) {
    ...
    this.setState(state => {
      var list = null;
      if (stepId !== "finish") {
        list = state.currentPage.page.content[componentIndex].steps[stepIndex][
          sec
        ] = value;
      } else {
  ...
  removeComponent(compId) {
    var compIndex = this.state.currentPage.page.content.findIndex(
      obj => obj.id === compId
    );
    this.setState(state => {
  ...
  onDragEnd = result => {
    ...
    const currentContent = this.state.currentPage.page.content;
    const comps = reorder(
      currentContent,
      source.index,
      destination.index
    ...
    if (source.droppableId === 'TOOLS' && destination.droppableId === 'contentBox')
    {
      const newComp = copy(
                        tools,
                        currentContent,
    ...
    return (
      <div className={classes.root}>
                ...
                >
                  ...
                  <Grid item>
                    <Switch
                      ...
                      checked={this.state.currentPage.page.external}
                      color="primary"
                      name="checkedC"
                    />
                  </Grid>
        ...
        >
          <Paper
            className={
              this.state.currentPage.page.external
                ? clsx(classes.disabled, classes.paper)
                : classes.paper
            }
          >
            <Title>{this.state.currentPage.page.text || ""}</Title>
            <Droppable droppableId="contentBox">
              {(provided, snapshot) => (
                <div className={snapshot.isDraggingOver ? classes.dragged :''} ref={provided.innerRef} {...provided.droppableProps}>
                  <Box height="25%" m={1}>
                  {content.map((comp, index) => (
                    <div key={index}>
                      <Draggable draggableId={comp.id.toString()} index={index}>
                        {(provided, snapshot) => (
                          <div
                            key={index}
                          ...
                          >
                            ...
                            >
                              ...
                              <Grid item>
                                <IconButton
                                  onClick={() => this.removeComponent(comp.id)}
                            ...
                            {(() => {
                              switch (comp.component) {
                                ...
                                case "expansion":
                                  return (
                                    <ExpansionEditor
                                      ExpansionText={this.expansionText}
                                      key={index + "76"}
                                      {...comp}
                                    />
                                  );
          ...
          <Drawer
            ...
            open={true}
          >
            <div className={classes.toolbarr}></div>
            
                  <EditorDrawer
                    OpenPage={this.openPage}
                    RemovePage={this.removeDialog}
                    RenamePage={this.renamePageDialog}
                    pages={pagesList}
                  />
